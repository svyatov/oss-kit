...
`skills/oss-audit/ecosystems.json` is the roster, the one machine-readable record of which distribution ecosystems the kit covers. Its `ecosystems` map holds each one's title, manifest, lockfiles, and registry facts, and its `sections` map declares the headings every skill that answers per ecosystem must carry. Seven skills answer: `oss-audit`, `oss-readme`, `oss-community`, `oss-ci`, `oss-harden`, `oss-publish`, and `oss-changelog`. Each keeps one file per ecosystem under `references/ecosystems/`, named for the registry where there is one. Adding an ecosystem means a roster entry plus one file in each of the seven, and `scripts/check-ecosystems.mjs` fails until all seven exist.
...
- R-PUB-01 through R-PUB-07: oss-kit ships through git, `npx skills add`, and the Claude Code plugin marketplace. The PUB preamble now splits into two tracks, a registry push and a tag-published registry that reads the repository, and this repository is on neither. It pushes to no registry, its releases carry only the source archives GitHub generates, and no registry resolves a package identity for it. Shipping through git tags does not by itself place a repository outside the area, because that is how a Go module publishes; the absent package identity is what does. So the whole area is out of scope by its preamble and its rules are not checked one at a time. Watch this if a release ever attaches a built file, because R-PUB-05 and R-PUB-06 would both start applying that day.
- R-CHG-05: no public item has been removed, and the rule now says a project that has removed none falls outside it.
- R-CHG-07: this repository encodes no major version in a package identity, because it has no package identity to encode one in. The rule reaches an ecosystem that carries the major version in the name a consumer writes, such as a Go module path ending `/v2`. oss-kit is installed by repository path, so nothing here changes shape at a major bump.
...
R-SEC-13 is met by two more rulesets on `refs/tags/v*`, read back with `gh api repos/svyatov/oss-kit/rulesets` rather than inferred from the write. `tags-immutable` carries `update` and `deletion` with `bypass_actors` empty and `current_user_can_bypass` reading `never`, so no principal here can move or delete a released tag, the owner included. `tags-creation` carries `creation` alone and exempts `RepositoryRole` `5`, Repository admin, at `always`, which is what lets a release be cut. The split is the whole point. A bypass list is granted per ruleset and not per rule, so one ruleset holding all three rules forces a choice between nobody creating a tag and the exempted actor also rewriting one. Rulesets targeting the same ref aggregate, and the most restrictive version of a repeated rule wins, so two rulesets give creation and immutability different bypass lists. Do not merge them back into one. Note the consequence for a release: a mistagged release cannot be repointed or deleted while `tags-immutable` is active, so the tag has to be right the first time.
...
R-SEC-11 is met on coverage and on reporting, and open on blocking. `gh api repos/svyatov/oss-kit/vulnerability-alerts -i` answers 204 and `dependabot_security_updates` reads `enabled`, but the dependency graph does not parse `bun.lock`, so `gh api repos/svyatov/oss-kit/dependency-graph/sbom` lists 10 `pkg:npm` packages against the 469 that `site/bun.lock` resolves, alongside 8 `pkg:githubactions` and 1 `pkg:github`. The `osv-scanner` workflow covers that residual: its `pull-request` job scans what the change introduces, and its `full-scan` job scans the repository weekly and reports. `full-scan` also answers `workflow_dispatch`, so a fix landed on `main` can be confirmed and its code scanning alerts closed with `gh workflow run osv-scanner.yml --ref main` instead of waiting for the cron.

What is open is the blocking half. The `main` ruleset requires six checks and `pull-request / osv-scan` is not among them, so the scan reports today and does not gate. It stays that way until a pull request from a fork runs it green, because a fork gets a read-only token while the called workflow requests `security-events: write`. If GitHub compares the fork-capped grant rather than the requested one, that job fails at startup, produces no check run at all, and a required check that never reports blocks the merge with nothing to read. Where the request sits moved in osv-scanner-action v2.5.0, from the called workflow's top level to its scan job, so the failure would now surface when that job is scheduled rather than before any job starts. That changes when the run dies, not whether it does, and the grant this repository has to make is unchanged. One green fork run settles it, and adding the `pull-request / osv-scan` context to the ruleset's `required_status_checks` closes the rule. Add it there and not to `code_scanning_tools`, which holds CodeQL and takes analysis tools rather than check contexts.
