...
Check: a prose checker exits 0 over `README.md`, files under `docs/`, `CONTRIBUTING.md`, `SECURITY.md`, `CODE_OF_CONDUCT.md`, and the entry text under `CHANGELOG.md` headings, reading only prose outside fenced and indented code blocks, inline code spans, and block quotes, and finding no em dash (U+2014), no en dash (U+2013), no emoji character, no heading outside sentence case except one preserved from an attributed third-party code of conduct, and none of the words robust, powerful, seamless, comprehensive, blazing, or effortless. `oss-writing` ships one at `scripts/check-tells.mjs` in its own installed directory, run as `node check-tells.mjs --rule R-DOC-05 <root>`, which reads files, needs nothing installed, and runs on Node 22 or later and on Bun; it exits 0 clean, 1 on a finding, and 2 without answering, which includes a root that is not a git checkout. An auditor without that skill installed, or holding an exit 2, marks this rule unknown rather than scoring it by reading.
