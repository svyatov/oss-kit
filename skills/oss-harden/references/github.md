...
`cooldown` delays a version update by the number of days given. Dependabot applies 3 days on its own where the block is absent entirely, and the same 3 days inside the block where `default-days` is absent. That default landed on github.com on 2026-07-14 and reaches GitHub Enterprise Server from 3.23, so an older instance still waits nothing. Write the number anyway, because a reader cannot tell an intended 3 from a forgotten key, and because the block is also how a repository opts out: a `cooldown` shorter than the platform default leaves the project worse off than writing nothing. Dependabot accepts 1 to 90 days, so a repository cannot write a zero, and every value below 3 it can write is slower than the block it replaced. Set 3 or more on this forge. `semver-major-days`, `semver-minor-days`, and `semver-patch-days` refine it for the ecosystems that track semantic versioning.

`include` and `exclude` decide what the number reaches, and they are where a cooldown goes hollow. Both take up to 150 entries with wildcards, and `exclude` wins where an entry appears in each. Omitting `include` applies the cooldown to every dependency, so adding one narrows it to what it names, and `exclude: ["*"]` removes the cooldown while leaving a block that reads as configured. Report what the block resolves to rather than that it is present, and use `exclude` only to name the specific dependencies a project has a reason to exempt, which is what GitHub's own guidance recommends. What the block does not do is delay a security update, which is the point: an advisory-driven fix still arrives the day it is published, so raising the cooldown costs the project nothing it wanted. Seven days is a defensible starting value and the standard asks only for at least a day; set it against how quickly the project actually merges bumps, since a cooldown longer than the gap between merges changes nothing.
