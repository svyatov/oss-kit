---
name: oss-harden
description: "Harden the security posture of an open source repository: pin CI actions to full commit SHAs, restrict workflow permissions, enable automated dependency updates, configure branch protection, enforce code owner review, and sign tags. Use when the user asks to harden a repo, improve supply chain security, fix an OpenSSF Scorecard finding, pin actions, or lock down CI. Covers GitHub and GitLab. Publishing belongs to oss-release."
license: MIT
---
...
This skill owns the security posture of the workflow and pipeline files two other skills also write into, and the boundary is the rule area, not a description of files. `oss-ci` decides what runs and when; it emits `uses:` lines as version tags and leaves `permissions:` off the jobs it writes, by design, so this skill has something to pin and scope. `oss-release` writes the publish job; it leaves the same `uses:` lines unpinned and the same jobs short of a `permissions:` block, and says as much in its own reference files. Treat every unpinned `uses:` line and every job missing a `permissions:` block in a workflow or pipeline these two skills wrote as work waiting for this skill, not as a defect in their output. `oss-scaffold` writes CODEOWNERS but does not enforce it; enforcing code owner review is a branch or merge request protection setting, which belongs here. Do not decide what a job runs, add a step, or choose a registry authentication flow while working from this skill; note that the project needs it and hand the work to `oss-ci` or `oss-release`.
